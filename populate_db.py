    ...
    with open("ex09_initial_data.json", "r") as file:
        ...
        for elm in json_data:
            if elm['model'] == "ex09.planets":
                ...
                planet = Planets.objects.get(pk=elm['fields']["homeworld"]) if elm['fields']["homeworld"] else None
                People.objects.create(id=elm['pk'], name=elm['fields']["name"], birth_year=elm['fields']["birth_year"], gender=elm['fields']["gender"], eye_color=elm['fields']["eye_color"], hair_color=elm['fields']["hair_color"], height=elm['fields']["height"], mass=elm['fields']["mass"], homeworld=planet, created=elm['fields']["created"], updated=elm['fields']["updated"])
